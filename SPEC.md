...
- SimpleFIN Bridge reports a transactions request spanning more than 45 days as a provider error ("Requested date range exceeds recommended range of 45 days"), and provider errors fail closed. Any window longer than 45 days, initial or recurring, is therefore requested as consecutive non-overlapping pages of at most 45 days (`start-date` inclusive, `end-date` exclusive, only the last recurring page open-ended) and merged before validation: account blocks by stable identity with the latest page's balance fields, transactions concatenated with a repeated id keeping its latest version, and provider errors from any page preserved. A failed page fails the whole fetch, so partial history is never imported.
...
- A closed account can be permanently deleted after typing its name. This is a deliberate exception to history retention and carries the Void History and Close guards (no row in a closed month, reconciled, a transfer leg, or referenced by a refund on another account). Its transactions, import identities, review items, reconciliations, file-import batches, and schedules are removed with it, and a SimpleFIN link bound to it is dropped. An "all conditions" rule requiring the account is removed and an "any condition" rule loses only that condition. A card's payment category is removed, or kept as a hidden ordinary category when it holds assignments so past months keep their totals. An `accountDeleted` audit event records the counts.
- Settings → Backup offers Erase All Data after typing ERASE. It deletes every Keychain credential referenced by any budget first, and only if that succeeds erases every database row except migration history, vacuums the file, removes the pre-upgrade safety copy next to the database, and returns to onboarding. Backups saved elsewhere are untouched.
